...
// This package implements a single Process struct that manages AI event loops
// identically for both coordinator and worker roles, replacing the need for
// separate WorkerProcess and CoordinatorProcess implementations.
package process
...
func (b *OutputBuffer) Last(n int) []string {
	b.mu.RLock()
	defer b.mu.RUnlock()
